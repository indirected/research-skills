...
Use `synthesize-literature` output if the user has it — the themes in the synthesis are often the right backbone for related work.
...
- Use `synthesize-literature` output if the user has it — themes there often map directly to related-work paragraphs.
- Use `read-paper` notes for the close neighbors — the "position vs. our work" section of each reading note is a starter for the differentiation sentence.
- If the user hasn't read the close neighbors deeply, send them back to `read-paper` first. Don't bluff-paraphrase papers based on training data.
