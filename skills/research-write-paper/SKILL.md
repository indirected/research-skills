...
If the user is working on a **rebuttal**, that's a different genre — invoke `research-respond-to-reviews` instead.
...
- **Don't drift from the frame.** If the paper's framing (from `research-frame-research`) says the contribution is X, all sections should serve X. A beautiful paragraph that's off-frame weakens the paper.
