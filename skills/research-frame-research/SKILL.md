...
If the user is trying to *generate* ideas, use `research-brainstorm-ideas` first. If they're trying to *review* a framing (theirs or someone else's), use `research-review-paper`. This skill is for sharpening a frame the user has already chosen to pursue.
...
Don't write the full section — that's `research-write-paper`'s job. Just shape the frame so it fits the genre.
