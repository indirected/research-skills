...
- For new experiments needed in response, hand off to `design-experiments` for the design, then run.
- For revision writing, hand off to `write-paper` (or specific section references therein).
- For deeper understanding of the original critique categories, `review-paper/references/top-10-rejection-reasons.md` maps reviewer concerns onto well-known failure modes.
