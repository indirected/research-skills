...
Produce structured reading notes following `assets/reading-notes-template.md`. The template keeps notes consistent across papers so they flow cleanly into `research-synthesize-literature` later, but the user decides where to save the file. Suggest a filename like `notes-{author}-{year}-{shortslug}.md` and a default location like `literature/` or wherever the user has been saving notes, but honor any path they give.
